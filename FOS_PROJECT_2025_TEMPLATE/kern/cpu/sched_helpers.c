...
void env_set_priority(int envID, int priority)
{
	...
	struct Env* samaproc = NULL;
	envid2env(envID, &samaproc, 0);
	if (samaproc == NULL) {
		release_kspinlock(&ProcessQueues.qlock);
		return;
	}
	if (samaproc->env_status == ENV_READY) {
		acquire_kspinlock(&ProcessQueues.qlock);
		//remove_from_queue(&(ProcessQueues.env_ready_queues[proc->priority]),proc);
		sched_remove_ready(samaproc);
		samaproc->priority = priority;
		samaproc->samawaiting_ticks = 0;
		// proc->time = ticks;
		sched_insert_ready(samaproc);
		release_kspinlock(&ProcessQueues.qlock);
	} else {
		samaproc->priority = priority;
	}
