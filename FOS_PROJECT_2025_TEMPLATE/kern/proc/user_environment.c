...
struct Env* env_create(char* user_program_name, unsigned int page_WS_size,
		unsigned int LRU_second_list_size, unsigned int percent_WS_pages_to_remove) {
	...
	pushcli();
	{
		...
		PROGRAM_SEGMENT_FOREACH(seg, ptr_program_start) {
			...
			LOG_STRING("===============================================================================");
			LOG_STATMENT(cprintf("SEGMENT #%d, size_in_file = %d, size_in_memory= %d, dest va = %x",segment_counter, seg->size_in_file,seg->size_in_memory, seg->virtual_address));
			LOG_STRING("===============================================================================");
...
/*============================OUR CODEEE================================*/
/*==================Functions Needed For env_free()======================*/
#if USE_KHEAP
void freeWSELementsEnvExitFunc(struct Env *envExitElementFreeFunc) {
	struct WorkingSetElement* WSElemToBeFreed;
	LIST_FOREACH_SAFE(WSElemToBeFreed, &envExitElementFreeFunc->page_WS_list,WorkingSetElement)
	{
		LIST_REMOVE(&envExitElementFreeFunc->page_WS_list, WSElemToBeFreed);
		kfree(WSElemToBeFreed);
	}
}
void freeActiveListEnvExitFunc(struct Env *envExitElementFreeFunc) {
	struct WorkingSetElement* WSElemToBeFreed;
	LIST_FOREACH_SAFE(WSElemToBeFreed, &envExitElementFreeFunc->ActiveList,WorkingSetElement)
	{
		LIST_REMOVE(&envExitElementFreeFunc->ActiveList, WSElemToBeFreed);
		kfree(WSElemToBeFreed);
	}
}
void freeSecondListEnvExitFunc(struct Env *envExitElementFreeFunc) {
	struct WorkingSetElement* WSElemToBeFreed;
	LIST_FOREACH_SAFE(WSElemToBeFreed, &envExitElementFreeFunc->SecondList,WorkingSetElement)
	{
		LIST_REMOVE(&envExitElementFreeFunc->SecondList, WSElemToBeFreed);
		kfree(WSElemToBeFreed);
	}
}
#endif
...
void env_free(struct Env *e) {
	...
	/*============================OURR CODEEEE================================*/
	/*===================Implemented In The Specific Rules====================*/
   // [2] Free the pages in the PAGE working set from the main memory
#if USE_KHEAP
	struct WorkingSetElement* OriginWS_env;
	LIST_FOREACH(OriginWS_env, &(e->page_WS_list)) {
		unmap_frame(e->env_page_directory, OriginWS_env->virtual_address);
	}
	// according to
	if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
		LIST_FOREACH(OriginWS_env, &(e->ActiveList)) {
			unmap_frame(e->env_page_directory, OriginWS_env->virtual_address);
		}
		LIST_FOREACH(OriginWS_env, &(e->SecondList))
		{
			unmap_frame(e->env_page_directory, OriginWS_env->virtual_address);
		}
	}
...
#if USE_KHEAP
	// Free the working set elements
	   freeWSELementsEnvExitFunc(e);
	if (isPageReplacmentAlgorithmLRU( PG_REP_LRU_LISTS_APPROX )) {
		freeActiveListEnvExitFunc(e);
		freeSecondListEnvExitFunc(e);
	}
	// Free reference stream list for OPTIMAL
	if ( isPageReplacmentAlgorithmOPTIMAL() ) {
		struct PageRefElement* PREF,* NextPREF;
		LIST_FOREACH_SAFE(PREF, & ( e-> referenceStreamList), PageRefElement) {
			LIST_REMOVE(& ( e-> referenceStreamList ), PREF );
			kfree(PREF);
		}
	}
	// Free prepaged VAs array
	...
	{
	    struct WorkingSetElement *ws = &e->__ptr_tws[i];
	    if (ws->virtual_address != 0 && ws->empty == 0)
	    {
	        uint32 sharedID = ws->virtual_address & 0x7FFFFFFF;
	        delete_shared_object(sharedID, (void*)ws->virtual_address);
	        ws->virtual_address = 0;
	        ws->empty = 1;
	    }
	}
	e->nPageIn = 0;
	...
	struct Channel *freeee = e->channel;
	if (e->channel != NULL) {
		wakeup_all(freeee);
		e->channel = NULL;
	}
	// [7] Free all TABLES from the main memory
	for (uint32 DENOP = 0; DENOP < PDX(USER_TOP); DENOP++) {
		if (e->env_page_directory[DENOP] & PERM_PRESENT) {
			uint32 TABLE_PAGE_ADDR = EXTRACT_ADDRESS(e->env_page_directory[DENOP]);
#if USE_KHEAP
			uint32* PTR2PTable = (uint32*) kheap_virtual_address(TABLE_PAGE_ADDR);
			kfree(PTR2PTable);
#else
			struct FrameInfo* table_frame = to_frame_info(TABLE_PAGE_ADDR);
			table_frame->references = 0;
			free_frame(table_frame);
#endif
			e->env_page_directory[DENOP] = 0;
		}
	}
...
#else
	struct FrameInfo* dir_frame = to_frame_info(e->env_cr3);
	dir_frame->references = 0;
	free_frame(dir_frame);
#endif
	e->env_page_directory = NULL;
	e->env_cr3 = 0;
	// Free kernel stack (Can't Call delete_user_kern_stack() it in the env_free())
	//delete_user_kern_stack() we implement it//
	if (e->kstack != NULL) {
		kfree(e->kstack);
		e->kstack = NULL;
	}
...
//		remaining_ws_pages = remaining_ws_pages < 6 ? remaining_ws_pages:6 ;
	...
	LOG_STATMENT(
			cprintf("copying data to allocated area VA %x from source %x",dst_ptr, src_ptr));
	while ((uint32) dst_ptr< (ROUNDDOWN((uint32 )vaddr, PAGE_SIZE)+ (*allocated_pages) * PAGE_SIZE) && ((uint32) dst_ptr < ((uint32) vaddr + seg->size_in_file))) {
		*dst_ptr = *src_ptr;
		dst_ptr++;
		src_ptr++;
	}
...
void* create_user_kern_stack(uint32* ptr_user_page_directory) {
	...
	uint32* bedaytElStackBase = kmalloc(KERNEL_STACK_SIZE);
	if (bedaytElStackBase == NULL) {
		panic("Failed to allocate memory for user kernel stack");
	}
	pt_set_page_permissions(ptr_user_page_directory, (uint32) bedaytElStackBase, 0, PERM_PRESENT);

	return bedaytElStackBase;
