...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	// ###### HEREIN
	uint32* p_page_table;
	get_page_table(directory, virtual_address, &p_page_table);

	if (p_page_table == NULL){

		return -1;
	}
	else {
		// Permission bits -> PERM_PRESENT, PERM_WRITEABLE, PERM_USER... etc)
		return p_page_table[PTX(virtual_address)];
	}
...
inline void pt_clear_page_table_entry(uint32* directory, uint32 virtual_address)
{
		...
		if(p_page_table == NULL){
			panic("Page table does not exist, can't call pt_clear_page_table_entry");
		} else {

			p_page_table[PTX(virtual_address)] = 0;
			tlb_invalidate (directory, (void*) virtual_address);

		}
	// ######### HEREIN
	//TODO: PRACTICE: fill this function.
	//Comment the following lineand virtual
...
inline uint32 virtual_to_physical(uint32* directory, uint32 virtual_address)
{
	...
	uint32 PTE_ = p_page_table[PTX(virtual_address)];
	if ((PTE_ & PERM_PRESENT) == 0){
		return -1;
	}

	uint32 physical_frame_address = PTE_ & 0xFFF;
